async function handle(state, action) {
  const rosettalib = {
    mulberry32: function(a) {
       ...
       * 
       * It takes as input a as it's random seed. And returns a function which when called, generates a random number between 0 and 1.
       * The function is pseudo random. Therefore if two different instances of a program call this function with the same seed, then
       * the sequence of random numbers generated by the returned function will be the same for both instances.
    ...
    xmur3: function xmur3(str) {
      /**
       * This is a hashing function. It can be used to convert a string (such as the current arweave block hash) into a seed for a
    ...
    defaultKnowledgeWallet: function () {
      /**
       * Returns a new empty wallet for knowledge tokens.
       */
      return {
        amount: 0,
        locked: [],
    ...
    defaultlockedknowledge: function (amount) {
      /**
       * Returns a new wallet for locked Rosetta.
       */
      return {
        rosetta: 0,
        knowledge: amount,
    ...
    loadTransactionToJson: async function (tx) {
      /**
       * Loads a file stored on the arweave in json format.
       * This function will attempt to load the file 10 times before giving up.
       */
      for (let i = 0; i < 10; i++) {
        try {
    ...
    loadBlock: async function (tx) {
      /**
       * This loads a block of data in json format.
       * 
       * The assumption for this function is that it will recieve either the block itself already loaded into json
       * format, or a string which is the transaction hash where the block is stored on the arweave.
       */
    ...
    getChunkOfBlocks: async function (blocks, fr, to) {
      /**
       * Returs a chunk of blocks.
       * 
       * @param blocks must be a list of either blocks in json format, or the string hash pointing to where the block is located.
       */
    ...
    getValuesInIndex: async function (tx, indexs, indexName) {
      /**
       * Some rosetta data is stored outside this contract on the arweave.
       * 
       * Such a transaction is stored as a jsonarray object. The location of which is determined by tx.
       * Each item in the jsonArray will have an index. This is defined by indexName. This
       * function will return every jsonobject in tx, whose indexname is contained in indexs.
       * 
       * @param tx The location of the indexed data.
       * @param indexs The set of objects to be retrieved.
       * @param indexName The member of the json objects which defines the ID.
       */
    ...
    queryIndex: async function (indexTx, indexs, indexName) {
       ...
       * Some rosetta data is stored outside this contract on the arweave.
       * 
       * indexTx is transaction hash on arweave. It points to a file with an index.
       * indexTx points to a jsonArray which contains json objects of the format
       * {
       *  first: number,
       *  last: number,
       *  tx: "an arweave transaction"
       * }
       * tx is a jsonarray stored on the arweave which contains a set of objects with ids between first and last.
       * 
       * tx points to a set of files stored on the arweave that contains json objects. This functions will return
       * every such object which has an index in indexs.
       */
      const indexJson = await this.loadTransactionToJson(indexTx);
      indexs.sort();
      const toload = [];
      let i = 0;
      // First we find every transaction which contains at least one object we are interested in.
      indexJson.array.forEach((element) => {
        let indexset = null;
        while (i < indexs.length) {
          // If the current index is smaller than the current element no object with the current index is in our set.
          if (indexs[i] < element.first) {
            i++
            continue;
          }
          // If the current index is both larger than first and less than equal to last. Then this index is in this
          // element's tx.
          if (indexs[i] <= element.last) {
            if (indexset == null) {
              indexset = set();
            }
            //add this index to the set of indexs we are interested in.
    ...
    juryselect: function (state, trial, rand) {
      ...
      while (selectedJury.length < trial.size) {
        const randnum = rand()
        const jurynum = Math.min(int(randnum * jurypool.length), jurypool.length-1)
        selectedJury.add(jurypool[jurynum]);
      }
    ...
    updateState: function (state) {
      const rand = rosettalib.mulberry32(rosettalib.xmur3(SmartWeave.block.indep_hash)())
    ...
    mainfunctions: {
      ...
      submitEvidence: function (state, action) {
        ...
        throw new ContractError(`No ongoing trial for ${action.input.paperid}`)
      },
